...
void *server_action(void *arg) {
  ...
  switch (operation) {
    ...
    case WRITE:
      ...
      sprintf(result_message, "server: WRITE -- value written is '%s'\n",
              operand);
      break;
    case MEETUP:
      ...
      sprintf(result_message, "server: MEETUP -- value shared is '%s'\n",
              operand);
      break;
    case DEBUG:
      printf("server: DEBUG\n");
...
int main(int argc, char *argv[]) {
  ...
  while (comm_fd = accept(listen_fd, (struct sockaddr *)NULL, NULL)) {
    ...
    if (pthread_create(&thread_pool[thread_index], 0, server_action, &comm_fd) <
        0) {
